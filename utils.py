...
class WebUtils:
    ...
    def serve_logs(self, handler):
        try:
            ...
            if not os.path.exists(log_file_path):
                subprocess.Popen(f"sudo tail -f /home/optaris-defense/OptarisDefense/data/logs/* > {log_file_path}", shell=True)
    ...
    def start_orchestrator(self, handler):
        try:
            optaris_defense_instance = self.shared_data.optaris_defense_instance
    ...
    def restart_optaris_defense_service(self, handler):
        try:
            command = "sudo systemctl restart optaris-defense.service"
            subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            handler.send_response(200)
            handler.send_header("Content-type", "application/json")
            handler.end_headers()
            handler.wfile.write(json.dumps({"status": "success", "message": "OptarisDefense service restarted successfully"}).encode('utf-8'))
        except subprocess.CalledProcessError as e:
            handler.send_response(500)
            handler.send_header("Content-type", "application/json")
            handler.end_headers()
